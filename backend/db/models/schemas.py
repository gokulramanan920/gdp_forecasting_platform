...
class ModelPrediction(Base):
    ...
    scenario = Column(String(20), default="baseline")
    
    # Quantile predictions
    predicted_gdp_median = Column(Numeric(15, 2))
    predicted_gdp_p10 = Column(Numeric(15, 2))
    predicted_gdp_p90 = Column(Numeric(15, 2))
    
    # Bootstrap (optional - can be NULL)
    bootstrap_mean = Column(Numeric(15, 2))
    bootstrap_std = Column(Numeric(15, 2))
    bootstrap_ci95_lower = Column(Numeric(15, 2))
    bootstrap_ci95_upper = Column(Numeric(15, 2))
    
    model_version = Column(String(50))
    dampening_factor = Column(Numeric(3, 2))
    created_at = Column(TIMESTAMP, default=datetime.datetime.utcnow)
    
    country = relationship("Country", back_populates="predictions")
    
    __table_args__ = (
        UniqueConstraint('country_code', 'year', 'scenario', 'model_version', name='uq_prediction'),
        Index('idx_predictions_country_year_scenario', 'country_code', 'year', 'scenario'),
        Index('idx_predictions_median', 'predicted_gdp_median'),
        Index('idx_predictions_p10', 'predicted_gdp_p10'),
        Index('idx_predictions_p90', 'predicted_gdp_p90'),
    )
